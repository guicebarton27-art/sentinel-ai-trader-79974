...
**Success criteria**
- 90%+ of executed trades display a rationale card.
- Users can trace a trade to the exact model version and input snapshot.
...
**Success criteria**
- All risk breaches are surfaced in <60 seconds.
- Recovery to last-known-good risk profile in <5 minutes.
...
**Success criteria**
- Average slippage improves by 15% vs baseline routing.
- Execution score card is generated for every filled order.
...
**Success criteria**
- 95% of orders route to top-2 venues by live score.
- Arbitrage fills show positive net P&L after fees.
...
**Success criteria**
- Strategy allocations update on a fixed cadence with audit trails.
- Underperforming strategies are auto-retired with transparent reason codes.
...
**Success criteria**
- 100% of trades have complete decision chains.
- Trade replay succeeds with identical outcomes for recorded inputs.
...
## Dependencies & Risks
- **Data quality**: replay accuracy depends on full input capture and versioning.
- **Latency budget**: richer explanations and telemetry must not block execution.
- **Exchange coverage**: multi-venue scoring requires reliable market data feeds.
